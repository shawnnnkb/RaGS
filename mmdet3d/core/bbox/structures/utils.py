...
def limit_period(val, offset=0.5, period=np.pi):
    ...
    def ensure_tensor(val):
        ...
        return val  # already a torch.Tensor
    val = ensure_tensor(val)
    period = ensure_tensor(period)
...
def rotation_3d_in_axis(points, angles, axis=0):
    ...
    """
    angles = torch.tensor(angles) if not isinstance(angles, torch.Tensor) else angles
    points = torch.tensor(points) if not isinstance(points, torch.Tensor) else points
