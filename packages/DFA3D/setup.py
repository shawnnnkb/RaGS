...
def get_extensions():
    ...
    if cuda_args and len(cuda_args)>0:
        ...
        extra_compile_args['nvcc'] = []
    extra_compile_args['nvcc'] = [
        '-gencode=arch=compute_70,code=sm_70',  # 适用于较旧显卡
        '-gencode=arch=compute_75,code=sm_75',  # Turing 架构，如 RTX 20 系列
        '-gencode=arch=compute_80,code=sm_80',  # Ampere 架构，如 RTX 30 系列
        '-gencode=arch=compute_86,code=sm_86'   # Ampere 架构，如 RTX 30 系列
        ]
