...
def parse_args():
    ...
    parser.add_argument(
        '--task',
        type=str,
        choices=['det', 'seg', 'multi_modality-det', 'RadarGS-det'],
...
def show_proj_bbox_img(idx,
                       ...
                       filename,
                       show=False,
                       is_nus_RadarGS=False):
    """Visualize 3D bboxes on 2D image by projection."""
    try:
        example = dataset.prepare_train_data(idx)
    except AttributeError:  # for RadarGS-3D datasets
...
def main():
    ...
    vis_task = args.task  # 'det', 'seg', 'multi_modality-det', 'RadarGS-det'
    ...
    for idx, data_info in enumerate(track_iter_progress(data_infos)):
        ...
        elif dataset_type in ['NuScenesRadarGSDataset']:
        ...
        if vis_task in ['multi_modality-det', 'RadarGS-det']:
            ...
            show_proj_bbox_img(
                ...
                file_name,
                show=args.online,
                is_nus_RadarGS=(dataset_type == 'NuScenesRadarGSDataset'))
