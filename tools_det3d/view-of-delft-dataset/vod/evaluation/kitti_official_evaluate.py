...
def get_official_eval_result(gt_annotations, dt_annotations, current_classes, pr_detail_dict=None, AP=11, custom_method=0):
    if custom_method == 0:
    ...
    if AP==40:
        for j, curcls in enumerate(current_classes):
            ...
            for i in range(1, 2):  # min_overlaps.shape[0]):
                if compute_aos:
                    if i == 1:
                        ret_dict['%s_aos_all' % class_to_name[curcls]] = mAPaos_R40[j, 0, 1]

                if i == 1:
                    ret_dict['%s_3d_all' % class_to_name[curcls]] = mAP3d_R40[
                        j, 0, 1]  # get j class, difficulty, second min_overlap
                    ret_dict['%s_bev_all' % class_to_name[curcls]] = mAPbev_R40[
                        j, 0, 1]  # get j class, difficulty, second min_overlap
