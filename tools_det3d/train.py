...
def parse_args():
    ...
    parser.add_argument('--config', default='./projects/RadarGS/configs/vod-RadarGS_4x4_24e_detect3d_pruning.py', help='train config file path')
...
def filter_state_dict(model, checkpoint):
    ...
    for k, v in checkpoint.items():
        if k in model_state:
            if model_state[k].shape == v.shape:
                filtered_ckpt[k] = v
        #     else:
        #         print(f"Skipping: {k} | shape mismatch ckpt: {v.shape}, model: {model_state[k].shape}")
        # else:
        #     print(f"Skipping: {k} | not in model")
    return filtered_ckpt

def transform_spconv1_spconv2(pretrained_model):
    if 'gs_encode.layers.2.layer.weight' in pretrained_model.keys(): pretrained_model['gs_encode.layers.2.layer.weight'] = pretrained_model['gs_encode.layers.2.layer.weight'].permute([1,2,3,4,0])
    if 'gs_encode.layers.5.layer.weight' in pretrained_model.keys(): pretrained_model['gs_encode.layers.5.layer.weight'] = pretrained_model['gs_encode.layers.5.layer.weight'].permute([1,2,3,4,0])
    if 'gs_encode.layers.8.layer.weight' in pretrained_model.keys(): pretrained_model['gs_encode.layers.8.layer.weight'] = pretrained_model['gs_encode.layers.8.layer.weight'].permute([1,2,3,4,0])
...
def load_pretrained_model(model, check_point_path, mapping_list):
    ...
    ckpt = state_dict
    ckpt = transform_spconv1_spconv2(ckpt)
    ...
    for k, v in ckpt.items():
        for old_key, new_key in mapping_list.items():
            if k.startswith(old_key):
                ...
                new_ckpt[new_k] = v
                break
    new_ckpt = filter_state_dict(model, new_ckpt)
...
def main():
    ...
    ######################################################################################
    if 'load_gauss_from' in cfg:
        print('load_gauss_from is exists, state_dict mapping, %s'%(cfg.load_gauss_from))
        mapping_list = {
            'gs_lifter': 'gs_lifter',
            'gs_encode': 'gs_encode',
            'gs_head2v': 'gs_head2v',
        ...
        model = load_pretrained_model(model, cfg.load_gauss_from, mapping_list)
    ######################################################################################
    if 'load_radar_from' in cfg:
        print('load_radar_from is exists, state_dict mapping, %s'%(cfg.load_radar_from))
        mapping_list = {
    ...
    ######################################################################################
    if 'load_depth_from' in cfg:
        print('load_depth_from is exists, state_dict mapping, %s'%(cfg.load_depth_from))
        mapping_list = {
            'depth_net': 'depth_net',
            'pvseg_net': 'pvseg_net'} # 'rangeview_foreground': 'pvseg_net'
        model = load_pretrained_model(model, cfg.load_depth_from, mapping_list)
    ######################################################################################
    if 'load_image_from' in cfg:
        print('load_image_from is exists, state_dict mapping, %s'%(cfg.load_image_from))
        mapping_list = {
            'img_backbone': 'img_backbone',
            'img_neck':'img_neck'}
        model = load_pretrained_model(model, cfg.load_image_from, mapping_list)
    ######################################################################################
    if 'load_det2d_from' in cfg:
        print('load_image_from is exists, state_dict mapping, %s'%(cfg.load_image_from))
        mapping_list = {
