...
from .kitti_data_utils_TJ4D import get_kitti_image_info, get_waymo_image_info
...
def create_kitti_info_file(data_path,
    ...
    val_small_img_ids = _read_imageset_file(str(imageset_folder / 'val_small.txt'))
    ...
    # no need for test in TJ4D because no test
    ...
    # kitti_infos_val_small = get_kitti_image_info(
    #     data_path,
    #     training=True,
    #     velodyne=True,
    #     calib=True,
    #     image_ids=val_small_img_ids,
    #     relative_path=relative_path)
    # _calculate_num_points_in_gt(data_path, kitti_infos_val_small, relative_path)
    # filename = save_path / f'{pkl_prefix}_infos_val_small.pkl'
    # print(f'Kitti info val small file is saved to {filename}')
    # mmcv.dump(kitti_infos_val_small, filename)
...
def export_2d_annotation(root_path, info_path, RadarGS3d=True):
    """Export 2d annotation from the info file and raw data.

    Args:
        root_path (str): Root path of the raw data.
        info_path (str): Path of the info file.
        RadarGS3d (bool): Whether to export RadarGS3d annotation. Default: True.
    """
    ...
    for info in mmcv.track_iter_progress(kitti_infos):
        coco_infos = get_2d_boxes(info, occluded=[0, 1, 2, 3], RadarGS3d=RadarGS3d)
        (height, width,
        ...
        for coco_info in coco_infos:
            ...
            coco_ann_id += 1
    if RadarGS3d:
        json_prefix = f'{info_path[:-4]}_RadarGS3d'
    else:
        json_prefix = f'{info_path[:-4]}'
    mmcv.dump(coco_2d_dict, f'{json_prefix}.coco.json')


def get_2d_boxes(info, occluded, RadarGS3d=True):
    ...
    Args:
        ...
        RadarGS3d (bool): Whether to get boxes with RadarGS3d annotation.
    ...
    for ann_idx, ann_rec in enumerate(ann_recs):
        ...
        # If RadarGS3d=True, add 3D annotations in camera coordinates
        if RadarGS3d and (repro_rec is not None):
